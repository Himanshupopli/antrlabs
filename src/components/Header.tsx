...
export default function Header({ currentView, onNavigate }: HeaderProps) {
  ...
  const navItems = [
    { label: "WORK", href: "#work", view: "work" as const, action: () => onNavigate("work") },
    { label: "TEAM", href: "#team", view: "team" as const, action: () => onNavigate("team") },
    { label: "IDEAS", href: "#ideas", view: "ideas" as const, action: () => onNavigate("ideas") },
    { label: "CONTACT", href: "#contact", view: "contact" as const, action: () => onNavigate("contact") }
  ];

  return (
    <>
      {/* Scroll Progress Bar */}
      {(currentView === "home" || currentView === "ideas" || currentView === "team") && (
        <motion.div
          className="fixed top-0 left-0 right-0 h-[2px] bg-[#FF4500] origin-left z-[60] pointer-events-none"
          style={{ scaleX }}
        />
      )}
      ...
      >
        <div className="max-w-7xl mx-auto px-6 md:px-12 flex justify-between items-center">
          {/* Typographic nav logo matching the provided image style */}
          <a
            id="nav-logo"
            href="#"
            onClick={(e) => {
          ...
          >
            ...
            <img
              src="https://lh3.googleusercontent.com/d/1xWOiLfm1Cv9c6Fxq-gP5VYPgNmhBLMIK"
              alt="ANTR Logo"
              referrerPolicy="no-referrer"
              className="h-4 sm:h-5 md:h-8 w-auto object-contain shrink-0 max-w-none aspect-auto select-none"
            />
          ...
          <nav id="desktop-nav" className="hidden md:flex items-center space-x-10">
            {navItems.map((item) => {
              const isActive = item.view !== null && currentView === item.view;
              return (
                <a
                  ...
                  className={`font-mono text-[13px] md:text-sm font-semibold tracking-[0.18em] transition-colors duration-300 relative py-2 group ${
                    isActive ? "text-white" : "text-neutral-300 hover:text-white"
                  }`}
                >
                  {item.label}
                  <span className={`absolute bottom-0 left-0 h-[2px] bg-[#FF4500] transition-all duration-300 ${
            ...
            <div className="flex justify-between items-center w-full">
              ...
              >
                <img
                  ...
                  alt="ANTR Logo"
                  referrerPolicy="no-referrer"
                  className="h-4 w-auto object-contain shrink-0 max-w-none aspect-auto select-none"
                />
              </a>
              <button
